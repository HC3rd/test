 ...
 * negtive number. The column numbers are paired and specify
...
int main()
{
	...
	char output[MAX_INPUT];   /* array fot output line */
 ...
 * the indicated columns. The output line is the NULL terminated,
 */
void rearrange( char *output, char const *input,
		int n_columns, int const columns[])
{
	...
	for (col = 0; col < n_columns; col += 2){
		...
		strncpy( output + output_col, input + columns[col+1],
				nchars);
		output_col += nchars;
